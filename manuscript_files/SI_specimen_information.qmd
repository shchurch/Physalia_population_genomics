...
sample_info <- read.delim("../data/sample_ids.tsv",header=T,stringsAsFactors=F) %>%
  ...
  select(ID,ocean,location,lat,long,date_collected,status,permit) %>% filter(status != "excluded") %>% arrange(ID)
colnames(sample_info) <- c("ID","ocean region","location","latitude","longitude","date collected","quality status","permit")
