---
title: "Population genomics of the bluebottle (Cnidaria: _Physalia_): Specimen info"
format:
  pdf:
    toc: false
...
sample_info <- read.delim("data/sample_ids.tsv",header=T,stringsAsFactors=F) %>%
  ...
  select(ID,ocean,location,lat,long,date_collected,status) %>% filter(status != "excluded") %>% arrange(ID)
colnames(sample_info) <- c("ID","ocean region","location","latitude","longitude","date collected","quality status")

sample_info %>%
  kable("latex",caption = "Specimen data.",booktabs=T,longtable=T) %>%
  kable_styling(bootstrap_options = "striped", position="center",font_size=7,latex_options=c("hold_status","repeat_header"))
```
...
![Specimen photos: cluster B1 from E Australia.](figures/Physalia_specimen_photos_AusU.png){width=98%}

![Specimen photos: cluster B1 from the S Indian and S Atlantic.](figures/Physalia_specimens_IndianU.png){width=98%}
...
![Specimen photos: cluster C2.](figures/Physalia_specimen_photos_population4.png){width=98%}
