...
plot(e$values[0:n_vectors],ylab="eigenvalues",xlab="k",pch=16,cex=0.5,cex.lab=0.75)
...
q <- read.delim(paste0(dir,name,".admix.",k,".Q"),header=F,sep=" ")  %>% 
    ...
    group_by(label) %>% 
    mutate(likely_assignment = paste0(name,":",PC[which.max(value)]),
           assignment_prob = max(value))
    
assignments <- q %>% distinct(ID,likely_assignment)

PC_info <- left_join(PC_info,assignments,by="ID") 

write.table(file=paste0("../data/",name,"_admix_assignments.tsv"),sep="\t",assignments,row.names=F,quote=F)
```
...
si <- sample_info %>% filter(ID %in% assignments$ID) %>% 
  ...
  mutate(lat = round(lat,0), lon = round(lon,0)) %>% group_by(lat,lon,likely_assignment,location) %>% tally()
...
g1 <- ggplot(data=world2) + geom_sf(fill = "light gray", colour = NA) +
  ...
  geom_label_repel(data=dtran,aes(geometry=geometry,label= likely_assignment,fill=likely_assignment),stat="sf_coordinates",color="white",
    size=4.5,max.overlaps=100,box.padding=0.3,segment.color="dark gray",segment.size=0.5)  + 
  geom_point(data=dtran,aes(geometry=geometry,color=likely_assignment,shape=location),
        size=5,alpha=0.75,fill="white",stat="sf_coordinates") + 
...
pdf(paste0(file="../figures/panels/",name,"_",k,"_map_assignments.pdf"),height=6,width=10,useDingbats=F)
print(g1)
dev.off()
```
...
fct_cols <- RColorBrewer::brewer.pal(name="Dark2",n=8)

admix <- read_q(5,dir,name)
pdf(paste0("../figures/panels/",name,"_",k,"_admixture.pdf"),height=8,width=35)
...
pdf(paste0("../figures/panels/",name,"_",k,"_regress_geo_PC.pdf"),width=8,height=4,useDingbats=F)
...
g1 <- ggplot(PC_info,aes(x = a*PC1, y = b*PC2, color = ocean, shape = location)) + 
  ...
  geom_point(size = 2.25, stroke = 1, alpha=0.75) + 
  scale_shape_manual(values = shapes) + 
...
g2 <- ggplot(PC_info %>% mutate(date = lubridate::mdy(date_collected)),aes(x = a*PC1, y = b*PC2, color = date, shape=location)) + 
  geom_vline(xintercept=0, linetype="dotted", alpha=0.2) +
  geom_hline(yintercept=0, linetype="dotted", alpha=0.2) +
  geom_point(size = 2.25, stroke = 1, alpha=1, fill="white") + 
  scale_shape_manual(values = shapes) + 
...
g4 <- ggplot(PC_info,aes(x = a*PC1, y = b*PC2, color = likely_assignment, shape = location, label=ID)) + 
  geom_vline(xintercept=0, linetype="dotted", alpha=0.2) +
  geom_hline(yintercept=0, linetype="dotted", alpha=0.2) +
...
pdf(paste0("../figures/panels/",name,"_PCA_admix_assignments.pdf"),width=3.5,height=2,useDingbats=F)
print(g4)
dev.off()
```
