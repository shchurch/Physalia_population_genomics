...
q <- left_join(q,cluster_IDs,by="likely_assignment") %>% 
  arrange(match(cluster,c("A","B1","B2","C1","C2")), desc(assignment_prob)) %>% 
  ungroup()
...
fct_cols <- c("dark cyan","dark orange","dark red","#9B59B6","#6DCFF6","dark blue","dodgerblue2")
names(fct_cols) <- cluster_IDs %>% arrange(match(cluster,names(cols))) %>% pull(likely_assignment)

admix <- read_q(n,dir,name)
...
```{r write_pops}
#write.table(assignments %>% select(ID,cluster) %>% arrange(ID),file="../data/subset.txt",col.names=F,quote=F,row.names=F)
```
...
pops <- fst %>% arrange(pop1,pop2) %>% distinct(pop1,pop2)
...
#     combo <- pops[p,]
#     thisPop <- strict(fst, pop1 == combo$pop1[[1]] & pop2 == combo$pop2[[1]]) %>% mutate(start = window_pos_1 -1, end = window_pos_2-1,value = avg_wc_fst,name=paste0("reg",window_pos_1)) %>% select(chromosome,start,end,value) %>%
...
#     pop <- unique(pi$pop)[p]
#     thisPop <- strict(pi, pop == pop) %>% mutate(start = window_pos_1 -1, end = window_pos_2-1,value = avg_pi,name=paste0("reg",window_pos_1)) %>% select(chromosome,start,end,value) %>%
#     mutate(value = ifelse(is.na(value),"",value))
...
summary_fst <- fst %>% 
  ...
  filter(!(pop1 %in% c("B1 SW Atlantic","C1 W Indian","B2 Central Pacific") | pop2 %in% c("B1 SW Atlantic","C1 W Indian","B2 Central Pacific")))
