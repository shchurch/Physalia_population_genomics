...
g1 <- ggplot(data=world2) + geom_sf(fill = "light gray", colour = NA) +
  geom_point(data=itran,aes(geometry=geometry), size = 0.5, color = "red", alpha=0.7,pch=16 ,stat="sf_coordinates") + 
  geom_point(data=dtran,aes(geometry=geometry, size = n), color = "blue", alpha=0.7,pch=16 ,stat="sf_coordinates") + 
...
png(paste0("../figures/panels/cov_",name,".png"),width=600,height=400)
plot(e$values[0:n_vectors],ylab="eigenvalues",xlab="k",pch=16,cex=1)
dev.off()
```

```{r PCA}
g4 <- ggplot(PC_info,aes(y=PC2,x=-1*PC1,label=ID)) +
  geom_point(size=2.5,alpha=0.5,pch=16,color="blue") + 
